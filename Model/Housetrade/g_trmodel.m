...
methods (Static)
  ...
  function [grad]=partial_deriv(mp, s, sol, parameter)
    ...
    ded=0;
    for tau=1:mp.ntypes;
      [du{tau}, dccp_scrap{tau}, dev{tau}, dv{tau}, dlogccp{tau}, dccp{tau}, dctp{tau}, dq{tau}, ded_tau] ...
        = g_trmodel.gradients_tau(mp, s, tau, sol, parameter);
      ded=ded+mp.tw(tau)*ded_tau;
    end
    fields={'u_tau', 'ccp_scrap_tau','ev_tau', 'v_tau', 'logccp_tau', 'ccp_tau', 'ctp_tau', 'q_tau', 'ed'};
    grad=cell2struct({du, dccp_scrap, dev, dv, dlogccp, dccp, dctp, dq, ded},fields, 2);
  end

  function [du, dccp_scrap, dev, dv, dlogccp, dccp, dctp, dq, ded]=gradients_tau(mp, s, tau, sol, parameter)
    ...
    if strcmp(parameter, 'prices')
      ...
      dbellman=g_trmodel.dbellman(mp, s, ev, ccp, ccp_scrap, ctp, delta, du, 'prices');
    else
      parameter_types=unique(mp.pvec_info.pnames_type);

      %daccprob=g_trmodel.daccprob(mp, s, parameter_types);
      np=mp.pvec_info.np;
      du=zeros(s.ns, s.nd,s.nd, np); 
    ...
    dv = g_trmodel.dv(mp, s, F, dev, du,ev, parameter_types);
    ...
    dctp=g_trmodel.dctp_dp(mp, s, F.notrans, dccp, delta);
  ...
  function [du, dccp_scrap]=du_dp(mp, s, tau, ccp_scrap)
    ...
    dpbuy=zeros(s.nd,s.nd, s.np); 
    dpup = zeros(s.nd,s.nd,s.np);
    % derivative of net sales price after transactions cost 
    dpsell=zeros(s.ns,s.nd, s.np); 
    
    for j=1:s.ns
        dpbuy([s.id.trade_used{:}],j,:)= eye(s.np)*(1+mp.ptranscost);
        dpup([s.id.trade_used{:}],j,:) = eye(s.np)*(1+mp.ptranscost) + mp.mum{tau}; 
        dpsell([s.is.house_ex_scrap{:}],j,:)=eye(s.np)*(1-mp.ptc_sale);
    end
  ...
  function [dev]=dbellman(mp, s, ev, ccp, ccp_scrap, ctp, delta, du, parameter_type) 
    ...
    elseif strcmp(parameter_type, 'alpha')
      ...
      dev=permute(sum(sum(ccp.*du, 3),2), [1,3,4,2]); % choice probability weighted sum of derivatives
    end
  end % end of dbellman

  function [dv]=dv(mp, s, F, dev, du, ev, parameter_types)
    ...
    np=size(du, 4);
    nusedcars=numel([s.is.house_ex_scrap{:}]);
    dv=zeros(s.ns,s.nd, s.nd, np); 

    % Step 1: take derivatives of F.notrade*ev and F.trade*ev (holding fixed F, varying ev)
    
    dev_keep = zeros(s.ns,s.ns, s.np);
    dev_trade_purge = zeros(s.ns,s.ns, s.np);
    dev_upgrade_purge = zeros(s.ns,s.ns, s.np);
    for j=1: s.np
        dev_keep(:,:,j)=F.notrans*dev(:,:,j); 
        dev_trade_purge(:,:,j)=F.trans*dev(:,:,j); 
        dev_upgrade_purge(:,:,j) = F.trans*dev(:,:,j); 
    end
    % Step 2: add derivative of F.notrade*ev and F.trade*ev (holding fixed ev, varying F)
    %if numel(daccprob)>1
    dev_keep=dev_keep + (s.dQ*ev);
    dev_trade_purge=dev_trade_purge + (s.dF*ev);
    dev_upgrade_purge = dev_upgrade_purge + (s.dF*ev);

    %end 
    
    % gradient of v when keeping
    dv([s.is.house_ex_scrap{:}], s.id.keep,s.id.keep,:) =du([s.is.house_ex_scrap{:}],s.id.keep,s.id.keep,:) ... 
                           + reshape(mp.bet*dev_keep([s.is.house_ex_scrap{:}],s.id.keep,:), [nusedcars, 1,1, np]);
    ...
    dv(:,[[s.id.trade{:}] s.id.purge],s.id.keep, :) = du(:,[[s.id.trade{:}] s.id.purge],s.id.keep,:) ...
                                 + reshape(mp.bet*dev_trade_purge, [s.ns, s.ns,1, np]);
    dv(:,s.id.keep,[[s.id.upgrade{:}] s.id.purge], :) = du(:,s.id.keep,[[s.id.upgrade{:}] s.id.purge],:) ...
                                 + reshape(mp.bet*dev_upgrade_purge, [s.ns, 1,s.ns, np]);
    for j=1:s.np
        dv(:,[[s.id.trade{:}] s.id.purge],[[s.id.upgrade{:}] s.id.purge], j) = du(:,[[s.id.trade{:}] s.id.purge],[[s.id.upgrade{:}] s.id.purge],j) ...
                                 + mp.bet*dev_upgrade_purge(:,:,j) + mp.bet*dev_trade_purge(:,:,j);
    end
    ...
    if any(strcmp(parameter_types, 'beta')) 
      ...
      dv([s.is.house_ex_scrap{:}], s.id.keep,idx)   =  dv([s.is.house_ex_scrap{:}], s.id.keep,idx)+  reshape(F.notrans([s.is.house_ex_scrap{:}],:)*ev, [nusedcars, 1, 1]); 
      dv(:, [[s.id.trade{:}] s.id.purge], idx)      =  dv(:, [[s.id.trade{:}] s.id.purge], idx) +  reshape(F.trans*ev, [1, s.ns, 1]);   
    end
  ...
  function [dh] = dh_dp(mp, s, q, delta, dccp, dq, tau)
    ...
    % indexed by s.ipt
      ...
      for j=1:mp.nhousetypes;
        dh(s.ipt.new{j},:)   =mp.tw(tau)*dqpt(s.is.scrap{j},:);
        dh(s.ipt.used{j},:)  =mp.tw(tau)*dqpt(s.is.house_ex_scrap{j},:);
        dh(s.ipt.nohouse,:)    =mp.tw(tau)*dqpt(s.is.nohouse,:);
      end
  end
  ...
  function [dmarketshares] = dmarketshares_dp(mp, s, dh_tau)
    ...
    % indexed by s.ipt
    np=size(dh_tau{1},2);
    dmarketshares=nan(mp.ntypes,mp.nhousetypes+1, np);
    for tau=1:mp.ntypes;
      for j=1:mp.nhousetypes;
        dmarketshares(tau,j,:)=sum(dh_tau{tau}(s.ipt.house{j},:),1);
      end
      dmarketshares(tau,mp.ncartypes+1,:)=dh_tau{tau}(s.ipt.nohouse,:);
    end
  end
  ...
  function [dctp_dp] = dctp_dp(mp, s, Q, dccp_dp, delta, daccprob)
    ...
    for ip=1:np;
        for j=1:s.ns
            dctp_dp(:,:,j,ip)= dccp_dp(:,s.id.keep,s.id.keep,ip).*Q   + dccp_dp(s.tr.state,s.tr.choice,j,ip)*Q; %+dccp_dp(s.tr.state,j,s.tr.choice_up,ip)*Q ;
  ...
  function dedvt=ded(mp, s, ccp, dccp, q, dq,ccp_scrap, dccp_scrap)
    ...
    dedvt=zeros(s.np, np);
    dDdp=sum(q.*dccp(:,[s.id.trade_used{:}],s.id.keep,:),1);
