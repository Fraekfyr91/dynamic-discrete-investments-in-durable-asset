...
methods (Static)
  ...
  function phouse = phouse_after_passthrough(phouse_raw, mp, ihouse, DOPRINT)
      ...
      if isfield(mp, 'passthrough')
          ...
          delta_tax = phouse_at_full_passthrough - pcar_baseline_full_pasthrough;
  ...
  function [s] = index(mp, abar_j)
    ...
    for j=1:mp.nhousetypes;
      s.tr.next_keep([s.is.clunker{j}])=[s.is.house{j}(1)];
    end
  ...
  function [psell] = psell(mp, s, price_j)     
    ...
    for j=1:mp.nhousetypes
      ...
      psell(s.is.house_ex_house{j})=[price_j{j}*(1-mp.ptc_sale)-mp.tc_sale-mp.tc_sale_age*house_age];
  ...
  function [ev1, ccp, dev] = bellman(mp, s, util, F, ev)
    ...
    v(:,s.id.purge)               =util(:,s.id.purge)   + mp.bet * ev(s.is.nohouse);                     
