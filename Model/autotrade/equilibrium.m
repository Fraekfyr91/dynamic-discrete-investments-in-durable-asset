...
properties (Constant)
  ...
  options= optimoptions('fsolve','TolFun',1e-5,'TolX',1e-6, 'Display','off');
  tolerance=1e-5;     % solution tolerance: equilibrium not considered solved if max(abs(ed)) > equilibrium.tolerance
end
...
methods (Static)
  function [sol]=solve(mp, s, p0)
    ...
    %              - F.notrans governs aging for current car.
  ...
  function [marketshares] = marketshares(mp, s, h_tau)
    ...
    %           The first mp.ncartypes columns holds the market shares in house type j= 1: mp.nhousetypes, 
    %           the last column holds the fraction of consumers in the outside good (i.e. with no house)
